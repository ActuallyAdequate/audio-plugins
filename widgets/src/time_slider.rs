...
use nih_plug_vizia::widgets::util::remap_current_entity_x_coordinate;
use nih_plug_vizia::widgets::util::remap_current_entity_x_t;
...
impl TimeSlider {
    ...
    /// the VIZIA wrapper.
    ///
    /// See [`TimeSliderExt`] for additional options.
        ...
        FMap: Fn(&Params) -> &P + Copy + 'static,
    {
        ...
        Self {
            ...
            scrolled_lines: 0.0,
            style: TimeSliderStyle::Centered,
            label_override: None,
        }
        .build(
            cx,
            ParamWidgetBase::build_view(params, params_to_param, move |cx, param_data| {
                Binding::new(cx, TimeSlider::style, move |cx, style| {
    ...
    fn text_input_view(cx: &mut Context, display_value_lens: impl Lens<Target = String>) {
        Textbox::new(cx, display_value_lens)
            ...
            .on_submit(|cx, string, success| {
                if success {
                    cx.emit(TimeSliderEvent::TextInput(string))
                } else {
                    cx.emit(TimeSliderEvent::CancelTextInput);
                }
            })
            .on_build(|cx| {
    ...
    fn slider_label_view<P: Param, L: Lens<Target = String>>(
        ...
        match (style, step_count) {
            (TimeSliderStyle::CurrentStepLabeled { .. }, Some(step_count)) => {
                HStack::new(cx, |cx| {
    ...
    fn compute_fill_start_delta<P: Param>(
        ...
        let step_count = param.step_count();
        let draw_fill_from_default = matches!(style, TimeSliderStyle::Centered)
            && step_count.is_none()
            && (0.45..=0.55).contains(&default_value);

        match style {
            TimeSliderStyle::Centered if draw_fill_from_default => {
            ...
            TimeSliderStyle::Centered | TimeSliderStyle::FromLeft => (0.0, current_value),
            TimeSliderStyle::CurrentStep { even: true }
            | TimeSliderStyle::CurrentStepLabeled { even: true }
            ...
            TimeSliderStyle::CurrentStep { .. } | TimeSliderStyle::CurrentStepLabeled { .. } => {
    ...
    fn compute_modulation_fill_start_delta<P: Param>(
        ...
        match style {
            ...
            TimeSliderStyle::Centered | TimeSliderStyle::FromLeft => {
...
impl View for TimeSlider {
    ...
    fn event(&mut self, cx: &mut EventContext, event: &mut Event) {
        event.map(|param_slider_event, meta| match param_slider_event {
            TimeSliderEvent::CancelTextInput => {
                self.text_input_active = false;
                cx.set_active(false);

                meta.consume();
            }
            TimeSliderEvent::TextInput(string) => {
                ...
                if cx.modifiers.alt() {
                    ...
                    if cx.modifiers.shift() {
                        ...
                        self.set_normalized_value_drag(
                            cx,
                            remap_current_entity_x_coordinate(cx, cx.mouse.cursorx),
                        );
                    }
                }
...
pub trait TimeSliderExt {
    ...
    fn set_style(self, style: TimeSliderStyle) -> Self;
...
impl TimeSliderExt for Handle<'_, TimeSlider> {
    fn disable_scroll_wheel(self) -> Self {
        self.modify(|param_slider: &mut TimeSlider| param_slider.use_scroll_wheel = false)
    }

    fn set_style(self, style: TimeSliderStyle) -> Self {
        self.modify(|param_slider: &mut TimeSlider| param_slider.style = style)
    }

    fn with_label(self, value: impl Into<String>) -> Self {
        self.modify(|param_slider: &mut TimeSlider| {
